...
func Test_ParsePackageAST(t *testing.T) {
	t.Run("ok", func(t *testing.T) {
		pkgs, err := ParsePackageAST(context.Background(), "./internal/testdata/ok")
		if err != nil {
		...
		pkg, exists := pkgs["github.com/krostar/test/internal/code/internal/testdata/ok"]
		if pkg == nil || !exists {
	...
	t.Run("pkg loaded with errors", func(t *testing.T) {
		pkgs, err := ParsePackageAST(context.Background(), "./internal/testdata/404")
		if err == nil || pkgs != nil {
	...
	t.Run("pkg not loaded", func(t *testing.T) {
		...
		pkgs, err := ParsePackageAST(ctx, "./internal/testdata/404")
		if err == nil || pkgs != nil {
			t.Fatalf("pkgs should be nil && err should be not nil: %v", err)
		}
